# CJDNS support in ITSAMEME Core

It is possible to run ITSAMEME Core over CJDNS, an encrypted IPv6 network that
...
done outside of ITSAMEME Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the ITSAMEME network and individual nodes.
...
before it will work with your ITSAMEME Core node. This is described in steps
...
## Run ITSAMEME Core with CJDNS

Once you are connected to the CJDNS network, the following ITSAMEME Core
...
When enabled, this option tells ITSAMEME Core that it is running in an
...
IPv6 local network. This helps ITSAMEME Core perform better address management:
...
CJDNS support was added to ITSAMEME Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `itsameme-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/itsamemedev/ITSAMEME/blob/master/doc/tor.md) for
details.

## CJDNS-related information in ITSAMEME Core

There are several ways to see your CJDNS address in ITSAMEME Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `itsameme-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `itsameme-cli getpeerinfo`).
