...
Below are some notes on how to build ITSAMEME Core for Windows.

The options known to work for building ITSAMEME Core on Windows are:
...
Note that for WSL the ITSAMEME Core source path MUST be somewhere in the default mount file system, for
example /usr/src/itsameme, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\itsameme`, for example:

    make install DESTDIR=/mnt/c/workspace/itsameme
