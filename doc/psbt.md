# PSBT Howto for ITSAMEME Core

Since ITSAMEME Core 0.17, an RPC interface exists for Partially Signed ITSAMEME
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/itsameme/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for ITSAMEME transactions that are not fully signed
...
#### Multisig with multiple ITSAMEME Core instances
...
ITSAMEME Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `itsameme-cli` is used.
...
Later, when *V* IAM has been received on *Amulti*, and Bob and Carol want to
