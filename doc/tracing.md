# User-space, Statically Defined Tracing (USDT) for ITSAMEME Core

ITSAMEME Core includes statically defined tracepoints to allow for more
...
**Note**: ITSAMEME Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to ITSAMEME Core
...
Multiple tools can list the available tracepoints in a `itsamemed` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in ITSAMEME Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/itsamemed
stap net        outbound_message 0x0000000000107c05 /src/itsamemed
stap validation block_connected  0x00000000002fb10c /src/itsamemed
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in ITSAMEME Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/itsamemed | grep NT_STAPSDT -A 4 -B 2
