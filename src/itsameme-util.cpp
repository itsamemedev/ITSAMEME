...
static void SetupITSAMEMEUtilArgs(ArgsManager &argsman)
{
...
static int AppInitUtil(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " itsameme-util utility version " + FormatFullVersion() + "\n";

        if (args.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  itsameme-util [options] [commands]  Do stuff\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
