...
static bool AppInit(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/itsameme.conf are parsed in qt/itsameme.cpp's main()
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  itsamemed [options]                     Start " PACKAGE_NAME "\n"
                "\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see itsamemed -h for a list of options.", argv[i])));
            }
        }

        // -server defaults to true for itsamemed but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect itsamemed signal handlers
    noui_connect();
