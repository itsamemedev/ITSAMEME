...
const char * const ITSAMEME_CONF_FILENAME = "itsameme.conf";
const char * const ITSAMEME_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //itsameme-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", ITSAMEME_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\ITSAMEME
    // macOS: ~/Library/Application Support/ITSAMEME
    // Unix-like: ~/.itsameme
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ITSAMEME";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/ITSAMEME";
#else
    // Unix-like
    return pathRet / ".itsameme";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", ITSAMEME_CONF_FILENAME));
}
