...
#include <qt/itsamemeaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a ITSAMEME address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new ITSAMEMEAddressEntryValidator(parent));
    widget->setCheckValidator(new ITSAMEMEAddressCheckValidator(parent));
}
...
bool parseITSAMEMEURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no itsameme: URI
    if(!uri.isValid() || uri.scheme() != QString("itsameme"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!ITSAMEMEUnits::parse(ITSAMEMEUnit::IAM, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseITSAMEMEURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseITSAMEMEURI(uriInstance, out);
}

QString formatITSAMEMEURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("itsameme:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ITSAMEMEUnits::format(ITSAMEMEUnit::IAM, info.amount, false, ITSAMEMEUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openITSAMEMEConf()
{
    ...
    /* Open itsameme.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ITSAMEME.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "ITSAMEME (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("ITSAMEME (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for ITSAMEME*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "itsameme.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("itsameme-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a itsameme.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=ITSAMEME\n";
        else
            optionFile << strprintf("Name=ITSAMEME (%s)\n", chain);
