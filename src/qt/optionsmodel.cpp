...
/** Call node.updateRwSetting() with ITSAMEME 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because ITSAMEME 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/itsamemedev/ITSAMEME/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because itsameme 22.x will not try to read these.
...
//! Convert enabled/size values to itsameme -prune setting.
...
//! Get pruning enabled value to show in GUI from itsameme -prune setting.
...
//! Get pruning size value to show in GUI from itsameme -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayITSAMEMEUnit")) {
        settings.setValue("DisplayITSAMEMEUnit", QVariant::fromValue(ITSAMEMEUnit::IAM));
    }
    QVariant unit = settings.value("DisplayITSAMEMEUnit");
    if (unit.canConvert<ITSAMEMEUnit>()) {
        m_display_itsameme_unit = unit.value<ITSAMEMEUnit>();
    } else {
        m_display_itsameme_unit = ITSAMEMEUnit::IAM;
        settings.setValue("DisplayITSAMEMEUnit", QVariant::fromValue(m_display_itsameme_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if itsameme.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_itsameme_unit);
...
{
    if (new_unit.isNull() || new_unit.value<ITSAMEMEUnit>() == m_display_itsameme_unit) return;
    m_display_itsameme_unit = new_unit.value<ITSAMEMEUnit>();
    QSettings settings;
    settings.setValue("DisplayITSAMEMEUnit", QVariant::fromValue(m_display_itsameme_unit));
    Q_EMIT displayUnitChanged(m_display_itsameme_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/itsamemedev/ITSAMEME/pull/8273
    ...
    // (https://github.com/itsameme-core/gui/issues/567).
