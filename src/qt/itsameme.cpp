...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<ITSAMEMEUnit>("ITSAMEMEUnit");
#else
    qRegisterMetaType<ITSAMEMEUnit>("ITSAMEMEUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. itsameme_de.qm (shortcut "de" needs to be defined in itsameme.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. itsameme_de_DE.qm (shortcut "de_DE" needs to be defined in itsameme.qrc)
...
static const char* qt_argv = "itsameme-qt";

ITSAMEMEApplication::ITSAMEMEApplication()
...
void ITSAMEMEApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the ITSAMEMEApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", ITSAMEMEGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void ITSAMEMEApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool ITSAMEMEApplication::createOptionsModel(bool resetSettings)
{
...
void ITSAMEMEApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new ITSAMEMEGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &ITSAMEMEGUI::quitRequested, this, &ITSAMEMEApplication::requestShutdown);
...
void ITSAMEMEApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void ITSAMEMEApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool ITSAMEMEApplication::baseInitialize()
{
    return node().baseInitialize();
}

void ITSAMEMEApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &ITSAMEMEApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &ITSAMEMEApplication::handleRunawayException);
    connect(this, &ITSAMEMEApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &ITSAMEMEApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void ITSAMEMEApplication::parameterSetup()
{
...
void ITSAMEMEApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void ITSAMEMEApplication::requestInitialize()
{
...
void ITSAMEMEApplication::requestShutdown()
{
...
void ITSAMEMEApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // itsameme: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ITSAMEMEGUI::handlePaymentRequest);
            connect(window, &ITSAMEMEGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void ITSAMEMEApplication::handleRunawayException(const QString &message)
{
...
void ITSAMEMEApplication::handleNonFatalException(const QString& message)
{
...
WId ITSAMEMEApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool ITSAMEMEApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ITSAMEMEGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(itsameme);
    Q_INIT_RESOURCE(itsameme_locale);
    ...
    /// 6-7. Parse itsameme.conf, determine network, switch to network specific
    ...
    // itsameme: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
