...
const std::string ITSAMEMEGUI::DEFAULT_UIPLATFORM =
...
ITSAMEMEGUI::ITSAMEMEGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &ITSAMEMEGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &ITSAMEMEGUI::showModalOverlay);
...
void ITSAMEMEGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a ITSAMEME address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and itsameme: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &ITSAMEMEGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &ITSAMEMEGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &ITSAMEMEGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your ITSAMEME addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified ITSAMEME addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed ITSAMEME Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed ITSAMEME Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a itsameme: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible ITSAMEME command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &ITSAMEMEGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &ITSAMEMEGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &ITSAMEMEGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &ITSAMEMEGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &ITSAMEMEGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &ITSAMEMEGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &ITSAMEMEGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &ITSAMEMEGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &ITSAMEMEGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &ITSAMEMEGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &ITSAMEMEGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &ITSAMEMEGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &ITSAMEMEGUI::showDebugWindow);
}

void ITSAMEMEGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void ITSAMEMEGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &ITSAMEMEGUI::setCurrentWalletBySelectorIndex);
...
void ITSAMEMEGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &ITSAMEMEGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &ITSAMEMEGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &ITSAMEMEGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &ITSAMEMEGUI::showProgress);
...
#ifdef ENABLE_WALLET
void ITSAMEMEGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void ITSAMEMEGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &ITSAMEMEGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &ITSAMEMEGUI::removeWallet);
...
WalletController* ITSAMEMEGUI::getWalletController()
{
    return m_wallet_controller;
}

void ITSAMEMEGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &ITSAMEMEGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &ITSAMEMEGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &ITSAMEMEGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &ITSAMEMEGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &ITSAMEMEGUI::incomingTransaction);
    connect(this, &ITSAMEMEGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void ITSAMEMEGUI::removeWallet(WalletModel* walletModel)
{
...
void ITSAMEMEGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void ITSAMEMEGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void ITSAMEMEGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void ITSAMEMEGUI::setWalletActionsEnabled(bool enabled)
{
...
void ITSAMEMEGUI::createTrayIcon()
{
...
void ITSAMEMEGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &ITSAMEMEGUI::toggleHidden);
...
void ITSAMEMEGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void ITSAMEMEGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void ITSAMEMEGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void ITSAMEMEGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void ITSAMEMEGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void ITSAMEMEGUI::openClicked()
{
...
void ITSAMEMEGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void ITSAMEMEGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void ITSAMEMEGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void ITSAMEMEGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void ITSAMEMEGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void ITSAMEMEGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void ITSAMEMEGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void ITSAMEMEGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to ITSAMEME network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void ITSAMEMEGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void ITSAMEMEGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void ITSAMEMEGUI::updateHeadersSyncProgressLabel()
{
...
void ITSAMEMEGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void ITSAMEMEGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &ITSAMEMEGUI::quitRequested);
...
void ITSAMEMEGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void ITSAMEMEGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void ITSAMEMEGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &ITSAMEMEGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &ITSAMEMEGUI::show);
                e->ignore();
            }
        }
    }
...
void ITSAMEMEGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void ITSAMEMEGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void ITSAMEMEGUI::incomingTransaction(const QString& date, ITSAMEMEUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(ITSAMEMEUnits::formatWithUnit(unit, amount, true));
...
void ITSAMEMEGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ITSAMEMEGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool ITSAMEMEGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool ITSAMEMEGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void ITSAMEMEGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void ITSAMEMEGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void ITSAMEMEGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void ITSAMEMEGUI::updateProxyIcon()
{
    std::string ip_port;
...
void ITSAMEMEGUI::updateWindowTitle()
{
...
void ITSAMEMEGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void ITSAMEMEGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ITSAMEMEGUI::detectShutdown()
{
...
void ITSAMEMEGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void ITSAMEMEGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(ITSAMEMEGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = ITSAMEMEGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void ITSAMEMEGUI::subscribeToCoreSignals()
{
...
void ITSAMEMEGUI::unsubscribeFromCoreSignals()
{
...
bool ITSAMEMEGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<ITSAMEMEUnit> units = ITSAMEMEUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const ITSAMEMEUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, ITSAMEMEUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const ITSAMEMEUnit u : ITSAMEMEUnits::availableUnits()) {
        menu->addAction(ITSAMEMEUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(ITSAMEMEUnit newUnits)
{
    setText(ITSAMEMEUnits::longName(newUnits));
}
