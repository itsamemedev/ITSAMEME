...
# The block reward of coinbaseoutput.nValue (50) IAM/block matures after
# COINBASE_MATURITY (100) blocks. Therefore, after mining 101 blocks we expect
# node 0 to have a balance of (BLOCKS - COINBASE_MATURITY) * 50 IAM/block.
...
class TestITSAMEMECli(ITSAMEMETestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `itsameme-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and itsameme-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and itsameme-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (IAM/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (IAM/kvB)']), network_info['relayfee'])
            ...
            # Mine a block to confirm; adds a block reward (50 IAM) to the default wallet.
            ...
            # Test itsameme-cli -generate with -rpcwallet in multiwallet mode.
