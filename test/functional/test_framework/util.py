...
def assert_fee_amount(fee, tx_size, feerate_IAM_kvB):
    """Assert the fee is in range."""
    assert isinstance(tx_size, int)
    target_fee = get_fee(tx_size, feerate_IAM_kvB)
    if fee < target_fee:
        raise AssertionError("Fee of %s IAM too low! (Should be %s IAM)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_IAM_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s IAM too high! (Should be %s IAM)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting IAM values"""
...
def get_fee(tx_size, feerate_iam_kvb):
    """Calculate the fee in IAM given a feerate is IAM/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_iam_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  IAM
...
def wait_until_helper(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `ITSAMEMETestFramework` or `P2PInterface` class ensures the timeout is
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "itsameme.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "itsameme.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "itsameme.conf")):
        with open(os.path.join(datadir, "itsameme.conf"), 'r', encoding='utf8') as f:
            for line in f:
