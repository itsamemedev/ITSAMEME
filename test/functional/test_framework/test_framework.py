...
TMPDIR_PREFIX = "itsameme_func_test_"
...
class ITSAMEMETestMetaClass(type):
    """Metaclass for ITSAMEMETestFramework.

    Ensures that any attempt to register a subclass of `ITSAMEMETestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'ITSAMEMETestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("ITSAMEMETestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("ITSAMEMETestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class ITSAMEMETestFramework(metaclass=ITSAMEMETestMetaClass):
    """Base class for a itsameme test script.

    Individual itsameme test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave itsamemeds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop itsamemeds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use itsameme-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_itsamemed = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "itsamemed" + config["environment"]["EXEEXT"],
        )
        fname_itsamemecli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "itsameme-cli" + config["environment"]["EXEEXT"],
        )
        fname_itsamemeutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "itsameme-util" + config["environment"]["EXEEXT"],
        )
        self.options.itsamemed = os.getenv("ITSAMEMED", default=fname_itsamemed)
        self.options.itsamemecli = os.getenv("ITSAMEMECLI", default=fname_itsamemecli)
        self.options.itsamemeutil = os.getenv("ITSAMEMEUTIL", default=fname_itsamemeutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: itsamemeds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("ITSAMEMERPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'itsamemed', self.options.itsamemed) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'itsameme-cli', self.options.itsamemecli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a itsamemed test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple itsamemed test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as itsamemed's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    itsamemed=self.options.itsamemed,
                    itsameme_cli=self.options.itsamemecli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in itsameme.conf
    ...
    def skip_if_no_itsamemed_tracepoints(self):
        """Skip the running test if itsamemed has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("itsamemed has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_itsamemed_zmq(self):
        """Skip the running test if itsamemed has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("itsamemed has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if itsameme-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("itsameme-wallet has not been compiled")

    def skip_if_no_itsameme_util(self):
        """Skip the running test if itsameme-util has not been compiled."""
        if not self.is_itsameme_util_compiled():
            raise SkipTest("itsameme-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if itsameme-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("itsameme-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether itsameme-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether itsameme-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_itsameme_util_compiled(self):
        """Checks whether itsameme-util was compiled."""
        return self.config["components"].getboolean("ENABLE_ITSAMEME_UTIL")
