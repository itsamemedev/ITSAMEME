...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing itsameme-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='itsameme-cli', help='itsameme-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='ITSAMEME address to which the faucet should send')
...
parser.add_argument('itsameme_cli_args', nargs='*', help='Arguments to pass on to itsameme-cli (default: -signet)')

args = parser.parse_args()

if args.itsameme_cli_args == []:
    args.itsameme_cli_args = ['-signet']


def itsameme_cli(rpc_command_and_params):
    argv = [args.cmd] + args.itsameme_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = itsameme_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = itsameme_cli(['getnewaddress', 'faucet', 'bech32'])
