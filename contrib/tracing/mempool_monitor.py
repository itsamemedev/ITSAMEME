...
""" Example logging ITSAMEME Core mempool events using the mempool:added,
...
def main(itsamemed_path):
    itsamemed_with_usdts = USDT(path=str(itsamemed_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    itsamemed_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    itsamemed_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    itsamemed_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    itsamemed_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[itsamemed_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/itsamemed")
        exit(1)
