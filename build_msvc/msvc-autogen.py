...
def parse_config_into_iam_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"ITSAMEME Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/itsameme_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/itsameme_config.h'), "w", encoding="utf8") as iam_config:
        iam_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='ITSAMEME-core msbuild configuration initialiser.')
    ...
    parse_config_into_iam_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/itsameme_config.h'), os.path.join(SOURCE_DIR, 'config/itsameme-config.h'))
